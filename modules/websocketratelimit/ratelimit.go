...
func (m *WebSocketRateLimit) Provision(ctx caddy.Context) error {
	...
	if m.UpBurstLimit == 0 {
		m.UpBurstLimit = 0
		m.UpByteRate = 0
		m.logger.Debug("upstream rate limit is disable")
	}
...
func (b *bidirectionalRateLimitedConn) Write(buf []byte) (n int, err error) {
	...
	if b.downloadLimiter != nil {
		if err := b.downloadLimiter.WaitN(b.ctx, len(buf)); err != nil {
			b.logger.Error("[+]Write limit error",
				zap.Int("current_size", len(buf)),
				zap.Int("downloadLimit", b.downloadLimiter.Burst()),
				zap.Error(err))
			return 0, err
		}
	}
...
func (b *bidirectionalRateLimitedConn) Read(buf []byte) (n int, err error) {
	...
	if b.uploadLimiter != nil {
		...
		if err != nil {
			b.logger.Error("[+]Read limit error",
				zap.Int("current_size", n),
				zap.Int("uploadLimit", b.uploadLimiter.Burst()),
				zap.Error(err))
			return n, err
		}
	}
